...
- `gpu_check.py`: Prints TensorFlow GPU devices and PyTorch MPS status, runs a small TensorFlow matmul on `/GPU:0` when present, a small PyTorch matmul on `mps` when available, and compares matmul timings CPU vs GPU for both frameworks.
